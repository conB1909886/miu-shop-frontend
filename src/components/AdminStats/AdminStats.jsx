...
import { getBase64, renderOptions, renderInventoryOptions } from '../../utils';
...
const AdminStats = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        return <span>{selled}</span>;
      },
    },
    {
      title: 'Doanh thu',
      sorter: (a, b) => {
        const selled = a.selled ?? 0;
        const valA = selled * a.price;
        const selledB = b.selled ?? 0;
        const valB = selledB * b.price;
        return valA - valB;
      },
      render: (_, record) => {
        const selled = record.selled ?? 0;
        const val = selled * record.price;
        return <span>{!isNaN(val) ? val : '-'}</span>;
      },
    },
    {
      title: 'Lợi nhuận',
  ...
  const handleOnchangeInventoryDetails = (value) => {
    setStateProductDetails({
      ...stateProductDetails,
      inventory: value,
    });
          ...
          >
            ...
            >
              <Select
                ...
                onChange={handleOnchangeInventoryDetails}
                options={renderInventoryOptions(allInventory?.data?.data)}
              />
            </Form.Item>
            <Form.Item
